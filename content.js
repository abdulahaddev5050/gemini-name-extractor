...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "SEND_INITIAL_PROMPT") {
    sendInitialPrompt(); // Send prompt only, no job data
  }
  if (request.action === "PROMPT_GEMINI") {
    sendJobData(request.job, request.fileId); // Send just the JSON data
  }
});
...
async function sendJobData(rawJobData, fileId) {
  try {
    ...
    // feedback_received_From_Freelancer → feedbackReceivedFromFreelancers
...
async function humanTypeAndSend(text, initialCount) {
    // Try multiple selectors for Gemini input (they change the UI sometimes)
    ...
    let attempts = 0;
    while (attempts < 15) {
        // First, check if Gemini already started generating
        const currentCount = document.querySelectorAll('.model-response-text').length;
        if (currentCount > initialCount) {
            sendLog("✅ Gemini started generating...");
            return; // EXIT IMMEDIATELY - don't click anything else!
        }

        // Check if there's a Stop button visible (means generation started)
        const stopBtn = document.querySelector('button[aria-label="Stop response"]') ||
                        document.querySelector('button[aria-label="Stop"]');
        if (stopBtn) {
            sendLog("✅ Gemini is generating (Stop button visible)...");
            return; // EXIT - generation is in progress!
        }

        // Only look for SEND button (not Stop button)
        ...
        if (sendBtn && !sendBtn.disabled && sendBtn.getAttribute('aria-disabled') !== 'true') {
            ...
            // Button might be disabled, try adding space to trigger it
    ...
    throw new Error("Failed to start Gemini response after 15 attempts. Check if Gemini page is open.");
}
